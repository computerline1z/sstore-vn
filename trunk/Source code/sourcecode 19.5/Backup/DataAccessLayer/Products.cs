    ...
    public class Products
    {
        private static ProductsInfo createObj(SqlDataReader dr, int stt)
        {
            try
            {
                ...
                info.Version = SqlObject.getString(dr, 2);
                info.SupplierID = SqlObject.getInt32(dr, 3);
                ...
                info.Status = SqlObject.getBool(dr, 8);
                info.PackageID = SqlObject.getInt32(dr, 9);
                info.ReleaseDate = SqlObject.getDateTime(dr, 10);
                info.UpdateDate = SqlObject.getDateTime(dr, 11);
                info.LinkDownload = SqlObject.getString(dr, 12);
                info.ViewCount = SqlObject.getInt32(dr, 13);
                info.DetailDescription = SqlObject.getString(dr, 14);
                info.TechDescription = SqlObject.getString(dr, 15);
                info.Tags = SqlObject.getString(dr, 16);
                //info.GroupID = SqlObject.getInt32(dr, 17);
                info.CategoryID = SqlObject.getInt32(dr, 17);
                //info.SubCategoryID = SqlObject.getInt32(dr, 19);
                info.IsBestSale = SqlObject.getBool(dr, 18);
                info.IsNew = SqlObject.getBool(dr, 19);
                info.IsHot = SqlObject.getBool(dr, 20);
                info.IsPublish = SqlObject.getBool(dr, 21);
                info.CreatedBy = SqlObject.getInt32(dr, 22);
                info.CreatedDate = SqlObject.getDateTime(dr, 23);
                info.PublishedBy = SqlObject.getInt32(dr, 24);
                info.PublishedDate = SqlObject.getDateTime(dr, 25);
                info.ShortDescription = SqlObject.getString(dr, 26);
                return info;
            }
            catch (Exception ex)
            {
